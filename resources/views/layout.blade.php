...
<head>
    ...
    <style>
        ...
        .gradient-oncf { background: linear-gradient(135deg, #1e3a5f 0%, #3b82f6 50%, #06b6d4 100%); }
...
<body class="h-full bg-gray-50 flex flex-col" x-data="{ mobileMenu: false }">
    ...
    <nav class="bg-white shadow-lg sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="hidden md:flex items-center space-x-8">
                    <a href="{{ url('/') }}" class="nav-link text-gray-700 hover:text-oncf-blue font-medium transition">Accueil</a>
                    <a href="{{ route('about') }}" class="nav-link text-gray-700 hover:text-oncf-blue font-medium transition">A Propos</a>
                    <a href="{{ route('contact') }}" class="nav-link text-gray-700 hover:text-oncf-blue font-medium transition">Contact</a>
                    <a href="{{ url('/cart') }}" class="relative text-gray-700 hover:text-oncf-blue font-medium transition flex items-center">
                    ...
                    @php
                        $userBilletsCount = \App\Models\Billet::whereHas('commande', function($q) {
                            $q->where('id_client', auth()->id());
                        })->count();
                    @endphp
                    @if($userBilletsCount > 0)
                    <a href="{{ route('my-tickets') }}" class="relative text-gray-700 hover:text-oncf-blue font-medium transition flex items-center">
                        <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/>
                        </svg>
                        Mes Billets
                        <span class="ml-1 bg-oncf-blue text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                            {{ $userBilletsCount }}
                        </span>
                    </a>
                    @endif
                    @if(strtoupper(auth()->user()->role) === 'ADMIN')
                    <a href="{{ route('admin.dashboard') }}" class="text-purple-600 hover:text-purple-800 font-medium transition flex items-center">
                        <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form method="POST" action="{{ url('/logout') }}" class="inline">
                        @csrf
                        <button type="submit" class="text-gray-700 hover:text-red-500 font-medium transition">Deconnexion</button>
                    </form>
                    @else
                    <a href="{{ url('/login') }}" class="text-gray-700 hover:text-oncf-blue font-medium transition">Connexion</a>
                    <a href="{{ url('/register') }}" class="bg-oncf-blue text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">Inscription</a>
                    @endauth
                </div>
        ...
        <div x-show="mobileMenu" x-transition class="md:hidden bg-white border-t">
            <div class="px-4 py-4 space-y-3">
                ...
                <a href="{{ route('contact') }}" class="block text-gray-700 hover:text-oncf-blue font-medium">Contact</a>
                ...
                <a href="{{ route('my-tickets') }}" class="block text-gray-700 hover:text-oncf-blue font-medium">
                    Mes Billets <span class="bg-oncf-blue text-white text-xs rounded-full px-2 py-1">{{ $userBilletsCount }}</span>
                </a>
                @endif
                @if(strtoupper(auth()->user()->role) === 'ADMIN')
                <a href="{{ route('admin.dashboard') }}" class="block text-purple-600 hover:text-purple-800 font-medium">Admin</a>
                @endif
    ...
    <div class="max-w-7xl mx-auto px-4 mt-4">
        <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg flex items-center justify-between" x-data="{ show: true }" x-show="show" x-transition>
    ...
    @if(session('error'))
    <div class="max-w-7xl mx-auto px-4 mt-4">
        <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg flex items-center justify-between" x-data="{ show: true }" x-show="show" x-transition>
            <div class="flex items-center">
                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
