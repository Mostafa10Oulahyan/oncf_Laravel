...
<div class="min-h-screen bg-gray-50 py-12">
    <div class="max-w-3xl mx-auto px-4">
        <div class="mb-8 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Modifier le Voyage</h1>
                <p class="text-gray-500 mt-1 font-mono text-sm">{{ $voyage->code_voyage }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8">
            <form method="POST" action="{{ route('admin.voyages.update', $voyage->id) }}">
                ...
                <!-- Promo Section -->
                <div class="mb-8 bg-orange-50 border border-orange-200 rounded-xl p-5"
                     x-data="{ isPromo: {{ old('is_promo', $voyage->is_promo) ? 'true' : 'false' }} }">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_promo" value="1" x-model="isPromo" class="sr-only peer"
                                   {{ old('is_promo', $voyage->is_promo) ? 'checked' : '' }}>
                    ...
                    <div x-show="isPromo" x-transition class="mt-3">
                        ...
                        <input type="number" name="price_promo"
                               value="{{ old('price_promo', $voyage->price_promo) }}"
                               min="0" step="0.01"
                               :required="isPromo"
